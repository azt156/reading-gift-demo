...
function pipeline(r){return '<div class="pipeline" aria-label="正式串接狀態"><span>本機預覽：已保存</span><span class="pending">樂寫共同資料：未串接</span><span class="pending">音檔轉寫：未串接</span><span class="pending">AI 輔助整理：未串接</span></div>';}
...
if($('#reading-form')){
 ...
 let desc={pending:'作品已保存在此瀏覽器。正式版規劃將文字、音檔、逐字稿與回饋統一保存至樂寫；音檔送出後才進行轉寫與 AI 輔助整理，最終由人工審核。此預覽尚未連接外部服務，可在管理者預覽中測試人工覆核。',revision:'請依下方回饋補充內容，完成後可再次送出。',approved:record.award==='reserved'?'已保留一份預覽名額，可以填寫測試用領取資料。':'本期預覽名額已滿，依通過順序列為候補；目前尚未取得領書資格。',claimed:'領取資料已保存在此瀏覽器。此操作只供流程確認，尚未安排寄送。'}[record.status];
